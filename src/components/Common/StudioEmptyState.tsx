    ...
    >
      <div className="relative mb-6">
        <motion.div 
          ...
          className={`absolute inset-0 blur-3xl rounded-full bg-mod-empty-glow`} 
        />
        <div className={`w-20 h-20 rounded-3xl flex items-center justify-center relative bg-mod-empty-icon-bg text-mod-empty-icon-text`}>
          {getIcon()}
        </div>
      </div>

      <h3 className="text-xl md:text-2xl font-black text-mod-surface-text-primary mb-3 tracking-tight">
        {getTitle()}
      </h3>
      <p className="text-mod-surface-text-secondary max-w-md text-[13px] md:text-sm leading-relaxed mb-8">
        {getDescription()}
      </p>
      ...
      {type === 'initial' ? (
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full justify-center">
          {onAction && actionLabel && (
            <button
              onClick={onAction}
              className={`w-full sm:w-auto px-8 py-3 text-white rounded-2xl font-bold transition-all flex items-center justify-center gap-2 text-sm hover:scale-[1.02] active:scale-95 bg-mod-empty-btn-bg hover:bg-mod-empty-btn-hover mod-empty-btn-shadow`}
            >
              {actionLabel}
            </button>
          )}
          {onSecondaryAction && secondaryActionLabel && (
            <button
              ...
              className={`w-full sm:w-auto px-6 py-3 bg-mod-surface-hover text-mod-surface-text-secondary rounded-2xl font-bold transition-all flex items-center justify-center gap-2 text-sm hover:bg-mod-empty-secondary-hover-bg text-mod-empty-secondary-text`}
            >
        ...
        <button
          onClick={onClear}
          className="px-8 py-3 bg-mod-surface-text-primary text-mod-surface-bg rounded-2xl font-bold hover:opacity-90 transition-all flex items-center gap-2 text-sm"
        >
