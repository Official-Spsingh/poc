      ...
      >
        {isOpen ? (
          ...
          <div className="relative">
            <Bot size={26} className="md:w-7 md:h-7 drop-shadow-sm" />
      ...
      <div className="flex-1 bg-white/40 backdrop-blur-3xl overflow-y-auto overflow-x-hidden custom-scrollbar relative">
        ...
        <main className="max-w-[1440px] mx-auto px-4 md:px-8 py-6 md:py-12 relative z-10">
          <div className="mb-8 md:mb-12 flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2 md:mb-4 tracking-tight">Welcome back, Shubham</h1>
              <p className="text-gray-500 text-base md:text-lg">Choose a tool to start building your next big idea.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
            {options.map((option) => (
              ...
              >
                ...
                <p className="text-gray-500 leading-relaxed mb-6 md:mb-8 text-xs md:text-sm">
                  {option.description}
                </p>
          ...
          <div className="mt-12 md:mt-20">
            <div className="flex items-center justify-between mb-6 md:mb-8">
              <h2 className="text-lg md:text-xl font-bold text-gray-900 flex items-center gap-2">
                <Clock size={20} className="text-blue-500" /> Recent Projects
              </h2>
              <button className="text-xs md:text-sm font-bold text-blue-700 hover:underline">View All Projects</button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {recentProjects.map((project) => (
                <div 
                  key={project.id} 
                  className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all cursor-pointer group flex flex-col"
                  onClick={() => {
          ...
          <div className="mt-12 md:mt-20 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
            <div className="bg-slate-900 rounded-2xl md:rounded-3xl p-6 md:p-8 text-white relative overflow-hidden group">
              <div className="relative z-10">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-violet-700 rounded-xl flex items-center justify-center mb-4 md:mb-6 shadow-lg shadow-violet-500/20">
                  <Sparkles size={20} className="md:w-6 md:h-6" />
                </div>
                <h3 className="text-xl md:text-2xl font-bold mb-2 md:mb-3">New: AI Agents</h3>
                <p className="text-slate-400 text-sm md:text-base mb-6 max-w-sm">Build agents that can think, reason, and act on your behalf.</p>
                <button className="inline-block px-5 md:px-6 py-2 md:py-2.5 bg-white text-slate-900 rounded-xl font-bold text-xs md:text-sm hover:bg-slate-50 transition-all font-sans" onClick={() => onNavigate('agent-home')}>Try AI Agents</button>
              </div>
              <div className="absolute -right-10 -bottom-10 w-48 h-48 md:w-64 md:h-64 bg-violet-700/20 rounded-full blur-3xl group-hover:scale-110 transition-transform" />
            </div>
                ...
                {[
                  ...
                  <div key={i}>
                    <div className="flex justify-between text-xs font-bold mb-2">
                      ...
                      <span className="text-gray-900">{stat.value} / {stat.total}</span>
                    </div>
